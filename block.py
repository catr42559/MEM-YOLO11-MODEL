...
######################################## MFEE ########################################
class EdgeEnhancer(nn.Module):  # 边缘信息增强小模块
...
class MFEE(nn.Module):  # 主体模块，先提取多尺度的信息(AdaptiveAvgPod)，再边缘信息增强(EdgeEnhancer)
    def __init__(self, inc, bins):
        ...
        for bin in bins:
            self.features.append(nn.Sequential(
                ...
                # 自适应平均池化，指定输出（H，W），输出固定尺寸，（bin下面给出四个参数从上往下是[3, 6, 9, 12]，所以有四层，比如第一行是3，是把特征图的尺寸缩放成3*3）
...
class MFEE(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(MFEE(c_, [3, 6, 9, 12]) for _ in range(n)))


class MFEE(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        self.m = nn.ModuleList(MFEE(self.c, self.c, 2, shortcut,
